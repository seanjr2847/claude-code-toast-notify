...
- `Stop` 훅 → ✅ 완료 토스트 (마지막 assistant 메시지 요약을 본문에)
...
3. `~/.claude/settings.json`에 훅 2개 + `preferredNotifChannel: notifications_disabled` 병합 (기존 설정은 보존, 수정 전 `settings.json.bak` 백업)
